...
class SimpleInvoice(SimpleDocTemplate):
    default_pdf_info = PDFInfo(title='Faktura', author='CiCiApp.com', subject='Faktura')
    precision = None
    ...
    def _invoice_info_data(self):
        if isinstance(self.invoice_info, InvoiceInfo):
            props = [('invoice_id', 'Faktura nr'), ('invoice_datetime', 'Leveransdatum'),
                     ('due_datetime', 'Förfallodatum'), ('company_expiration_fee', 'Dröjsmålsränta')]
    ...
    def _build_invoice_info(self):
        ...
        if invoice_info_data:
            self._story.append(Paragraph('Faktura', self._defined_styles.get('RightHeading1')))
    ...
    def _service_provider_data(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo):
            props = [('vat_tax_number', 'Org. nummer'), ('street', 'Address'), ('post_code', 'Post nr'), ('city', 'Stad'), ('state', 'Ort'),
                     ('country', 'Land')]
    ...
    def _client_info_data(self):
        ...
        props = [('client_id', 'Kund nr'), ('name', 'Namn'), ('street', 'Address'), ('post_code', 'Postnummer'), ('city', 'Stad'), ('state', 'Ort'),
                 ('country', 'Land'), ('email', 'E-post')]
    ...
    def _build_service_provider_and_client_info(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo) and isinstance(self.client_info, ClientInfo):
            ...
            # table_data = [
            #     [
            #         Paragraph('Leverantör', self._defined_styles.get('Heading1')), '',
            #         '',
            #         Paragraph('Mottagare', self._defined_styles.get('Heading1')), ''
            #     ]
            # ]
            table_data = [
                [
                    Paragraph(self.service_provider_info.name, self._defined_styles.get('Heading1')), '',
                    '',
                    Paragraph('Mottagare', self._defined_styles.get('Heading1')), ''
                ]
            ]
    ...
    def _item_data_and_style(self):
        ...
        self._story.append(
            ...
            Paragraph('', self._defined_styles.get('Heading1'))
        )

        item_data_title = ('Namn', 'Beskrivning', 'Antal', 'á pris', 'Kostnad')
